...
public class Tokenizer implements Iterator<Token> {
	...
	private Token curTok = new ErrorToken("Tokenizer has not yet begun reading", -1);
	...
	private void lexOneToken() throws IOException, EOFException {
		...
		case '*':
			...
			break;
		/*
		 * case '/': setNextTokenAndReset(DIV); tokenReady = false; if
		 * (peek().getType().equals(DIV)){ lineNo++; lexOneToken(); } tokenReady
		 * = true; break;
		 */
		case '<':
			lexLAngle();
		...
		case ':':
			if (consume('='))
				setNextTokenAndReset(ASSIGN);
			break;
		case '!':
			if (consume('='))
				setNextTokenAndReset(NE);
			break;
		default:
	...
	private void lexLAngle() throws IOException, EOFException {
		...
		if (c == -1)
			...
			if (cc == '=')
				...
				setNextTokenAndResetWith(LT, cc);
		}
	}
	...
	private void lexRAngle() throws IOException, EOFException {
		...
		if (c == -1)
			...
			if (cc == '=')
				...
				setNextTokenAndResetWith(GT, cc);
		}
	}
	...
	private void lexslash() throws IOException, EOFException {
		int c;
		for (c = nextChar(false); c == 47; c = nextChar(false))
			buf.append((char) c);
		...
		if (tt != null) {
			if (tt.equals(DIV)) {
				...
				if (ch == 10) {
					lineNo++;
					peek(); // Surround with try catch to handle extra lines at
					// the end?
				} else {
					encounteredEOF();
				}
